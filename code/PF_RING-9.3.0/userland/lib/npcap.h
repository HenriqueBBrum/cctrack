...
typedef void npcap_hash_filter_t;
...
#if !defined(__FreeBSD__) && !defined(__APPLE__) && !defined(__ARM_ARCH)
#include "pfring.h"
#endif
...
typedef struct timeline_extract_multi_handle timeline_extract_multi_handle_t;

/* single npcap extraction */
npcap_extract_handle_t * npcap_extract_open(char *pcap_path, char *index_path,
	nbpf_tree_t *nbpf_filter,
	struct bpf_program *pcap_filter, u_int8_t enable_pcap_post_filter,
	npcap_hash_filter_t *hash_filter);

struct pcap_file_header *npcap_extract_header(npcap_extract_handle_t *handle);

int npcap_extract_next(npcap_extract_handle_t *handle,
	struct pcap_disk_pkthdr **extracted_hdr, u_char **extracted_pkt);

void npcap_extract_close(npcap_extract_handle_t *handle);

/* timeline extraction */
timeline_extract_handle_t *timeline_extract_open(char *timeline_path,
	u_int64_t begin_epoch_nsec, u_int64_t end_epoch_nsec,
	nbpf_tree_t *nbpf_filter, struct bpf_program *pcap_filter, u_int8_t enable_pcap_post_filter,
	npcap_hash_filter_t *hash_filter);

struct pcap_file_header *timeline_extract_header(timeline_extract_handle_t *handle);

int timeline_extract_next(timeline_extract_handle_t *handle, struct pcap_disk_pkthdr **extracted_hdr,
	u_char **extracted_pkt, u_int64_t *match_epoch_nsec);

void timeline_extract_close(timeline_extract_handle_t *handle);

/* multi-timeline extraction (sorted merge across multiple timelines) */
timeline_extract_multi_handle_t *timeline_extract_open_multi(char **timeline_paths,
	u_int32_t num_timelines,
	u_int64_t begin_epoch_nsec, u_int64_t end_epoch_nsec,
	nbpf_tree_t *nbpf_filter, struct bpf_program *pcap_filter,
	u_int8_t enable_pcap_post_filter,
	npcap_hash_filter_t *hash_filter);

struct pcap_file_header *timeline_extract_header_multi(timeline_extract_multi_handle_t *handle);

int timeline_extract_next_multi(timeline_extract_multi_handle_t *handle,
	struct pcap_disk_pkthdr **extracted_hdr, u_char **extracted_pkt,
	u_int64_t *match_epoch_nsec);

void timeline_extract_close_multi(timeline_extract_multi_handle_t *handle);
