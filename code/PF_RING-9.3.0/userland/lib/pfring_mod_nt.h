...
int  pfring_nt_recv_pkt(pfring *ring, u_char** buffer, u_int buffer_len, struct pfring_pkthdr *hdr, u_int8_t wait_for_incoming_packet);
int  pfring_nt_recv_burst(pfring *ring, pfring_packet_info *packets, u_int8_t num_packets, u_int8_t wait_for_packets);
int pfring_nt_send_last_rx_packet(pfring *ring, int tx_interface_id);
...
int  pfring_nt_recv_chunk_burst(pfring *ring, pfring_packet_info *packets, u_int8_t num_packets, u_int8_t wait_for_packets);
int  pfring_nt_recv_chunk(pfring *ring, void **chunk, pfring_chunk_info *chunk_info,  u_int8_t wait_for_incoming_chunk); 
int pfring_nt_recv_flow(pfring *ring,  pfring_flow_update *flow, u_int8_t wait_for_flows);
u_int32_t pfring_nt_get_interface_speed(pfring *ring);
pfring_if_t *pfring_nt_findalldevs();
int pfring_nt_add_hw_rule(pfring *ring, hw_filtering_rule *rule);
