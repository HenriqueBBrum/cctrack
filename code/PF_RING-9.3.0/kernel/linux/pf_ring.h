...
typedef struct {
  ...
  /* Map ifindex to pf device idx (used for num_rings_per_device) */
...
struct pfring_hooks {
  u_int32_t magic; /* It should be set to PF_RING and it MUST be the first field */
...
typedef pf_ring_device ring_device_element;
...
static void register_plugin(struct pfring_plugin_registration *reg_info) {
  ...
  do_register_pfring_plugin(reg_info);
}

static void unregister_plugin(int plugin_id) {
  do_unregister_pfring_plugin((u_int16_t)plugin_id);
}
