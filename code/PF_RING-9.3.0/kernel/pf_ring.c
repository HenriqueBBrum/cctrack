...
MODULE_PARM_DESC(quick_mode, "(deprecated)");
...
static int ring_proc_get_plugin_info(struct seq_file *m, void *data_not_used)
{
  ...
  if(!plugin_registration_size) {
    seq_printf(m, "No plugin registered\n");
    return 0;
  }
...
static int ring_proc_plugins_info_open(struct inode *inode, struct file *file)
{
  return single_open(file, ring_proc_get_plugin_info, PDE_DATA(inode));
}

#if(LINUX_VERSION_CODE < KERNEL_VERSION(5,6,0))
static const struct file_operations ring_proc_plugins_info_fops = {
  .owner = THIS_MODULE,
  .open = ring_proc_plugins_info_open,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};
#else
static const struct proc_ops ring_proc_plugins_info_fops = {
  .proc_open = ring_proc_plugins_info_open,
  .proc_read = seq_read,
  .proc_lseek = seq_lseek,
  .proc_release = single_release,
};
...
static void ring_proc_init(pf_ring_net *netns)
{
  ...
  ring_proc_plugins_info = proc_create(PROC_PLUGINS_INFO,
                                       ...
                                       &ring_proc_plugins_info_fops /* file operations */);
  ...
  if(netns->proc == NULL || ring_proc_plugins_info == NULL) {
...
static void ring_proc_term(pf_ring_net *netns)
{
  ...
  remove_proc_entry(PROC_PLUGINS_INFO, netns->proc_dir);
...
success:
  ...
  //printk("rule->rule.extended_fields.filter_plugin_id=%d\n", rule->rule.extended_fields.filter_plugin_id);
  ...
  if((rule->rule.extended_fields.filter_plugin_id > 0)
    ...
    *last_matched_plugin = rule->rule.extended_fields.filter_plugin_id;
    hdr->extended_hdr.parsed_pkt.last_matched_plugin_id = rule->rule.extended_fields.filter_plugin_id;
  }
  ...
  if((rule->rule.plugin_action.plugin_id != NO_PLUGIN_ID)
    ...
    rc = plugin_registration[rule->rule.plugin_action.plugin_id]->pfring_plugin_handle_skb
                                  ...
                                  int offset, void *plugin_mem)
{
  ...
  if(pfr->kernel_consumer_plugin_id
     && pfr->kernel_consumer_plugin_id < MAX_PLUGIN_ID
...
static void free_filtering_rule(sw_filtering_rule_element * entry, u_int8_t freeing_ring)
{
  ...
  if(entry->rule.extended_fields.filter_plugin_id > 0
     && entry->rule.extended_fields.filter_plugin_id < MAX_PLUGIN_ID
     && plugin_registration[entry->rule.extended_fields.filter_plugin_id] != NULL
     && plugin_registration[entry->rule.extended_fields.filter_plugin_id]->pfring_plugin_register != NULL)
  ...
  if(entry->rule.plugin_action.plugin_id > 0
     && entry->rule.plugin_action.plugin_id < MAX_PLUGIN_ID
     && plugin_registration[entry->rule.plugin_action.plugin_id] != NULL
     && plugin_registration[entry->rule.plugin_action.plugin_id]->pfring_plugin_register != NULL)
  ...
  if(entry->plugin_data_ptr != NULL) {
    if(entry->rule.plugin_action.plugin_id > 0
       && entry->rule.plugin_action.plugin_id < MAX_PLUGIN_ID
       && plugin_registration[entry->rule.plugin_action.plugin_id] != NULL
       && plugin_registration[entry->rule.plugin_action.plugin_id]->pfring_plugin_free_ring_mem != NULL) {
...
static void free_sw_filtering_hash_bucket(sw_filtering_hash_bucket * bucket)
{
  ...
  if(bucket->rule.plugin_action.plugin_id > 0
     && bucket->rule.plugin_action.plugin_id < MAX_PLUGIN_ID
     && plugin_registration[bucket->rule.plugin_action.plugin_id] != NULL
     && plugin_registration[bucket->rule.plugin_action.plugin_id]->pfring_plugin_register != NULL)
  ...
  if(add_rule) {
    if(rule->rule.plugin_action.plugin_id != NO_PLUGIN_ID) {
      if(rule->rule.plugin_action.plugin_id >= MAX_PLUGIN_ID
         || plugin_registration[rule->rule.plugin_action.plugin_id] == NULL) {
        debug_printk(2, "Invalid action plugin [id=%d]\n", rule->rule.plugin_action.plugin_id);
        return(-EFAULT);
      }
    }
  }
  ...
  if(pfr->sw_filtering_hash[hash_idx] == NULL) {
    ...
    if(add_rule) {
      ...
      if(rule->rule.plugin_action.plugin_id > 0
         && plugin_registration[rule->rule.plugin_action.plugin_id] != NULL
         && plugin_registration[rule->rule.plugin_action.plugin_id]->pfring_plugin_register != NULL) {
...
static int add_sw_filtering_rule_element(struct pf_ring_socket *pfr, sw_filtering_rule_element *rule)
{
  ...
  if(rule->rule.extended_fields.filter_plugin_id > 0) {
    if(rule->rule.extended_fields.filter_plugin_id >= MAX_PLUGIN_ID)
      return(-EFAULT);
    if(plugin_registration[rule->rule.extended_fields.filter_plugin_id] == NULL)
      return(-EFAULT);
  }
                        ...
                        sw_filtering_hash_bucket **p_hash_bucket)
{
  ...
  if(hash_found) {
    ...
    if(hash_found
      ...
      rc = plugin_registration[hash_bucket->rule.plugin_action.plugin_id]->pfring_plugin_handle_skb
        ...
        *pass_pkt = 1;
        ...
        if(hash_bucket != NULL) {
          if(last_matched_plugin
            ...
            debug_printk(2, "pfring_plugin_add_rule(entry, hdr, hash_bucket) done!\n");
          } else {
            hash_bucket->rule.vlan_id = hdr->extended_hdr.parsed_pkt.vlan_id;
                           ...
                           u_int32_t num_rx_channels)
{
  ...
  */
  ...
  if(pfr->sw_filtering_hash != NULL) {
    ...
    hash_found = check_perfect_rules(skb, pfr, hdr, &pass_pkt, displ,
                     parse_memory_buffer, &free_parse_mem,
                     &last_matched_plugin, &hash_bucket);
  ...
  if((!hash_found) && (pfr->num_sw_filtering_rules > 0)) {
    if(check_wildcard_rules(skb, pfr, hdr, &pass_pkt, displ,
                            parse_memory_buffer, &free_parse_mem,
                            &last_matched_plugin) != 0)
      pass_pkt = 0;
  }
  ...
  if(pass_pkt) { /* We accept the packet: it needs to be queued */
    ...
    if(hdr->caplen > 0) {
      ...
      if((last_matched_plugin > 0)
        ...
        offset = parse_memory_buffer[last_matched_plugin]->mem_len;
      ...
      rc = add_pkt_to_ring(skb, real_skb, pfr, hdr, displ, channel_id, offset,
                           (last_matched_plugin > 0) ? parse_memory_buffer[last_matched_plugin]->mem : NULL);
    }
  }
...
static int register_plugin(struct pfring_plugin_registration *reg)
{
  ...
  printk("[PF_RING] registered plugin [id=%d][max=%d][%pK]\n",
...
static int unregister_plugin(u_int16_t pfring_plugin_id)
{
  u_int32_t last_list_idx = 0;
  struct sock *sk;
  int i;
  ...
  while(sk != NULL) {
    ...
    list_for_each_safe(ptr, tmp_ptr, &pfr->sw_filtering_rules) {
      sw_filtering_rule_element *rule = list_entry(ptr, sw_filtering_rule_element, list);

      if(rule->rule.plugin_action.plugin_id == pfring_plugin_id
         || rule->rule.extended_fields.filter_plugin_id == pfring_plugin_id) {
        ...
        if(rule->rule.extended_fields.filter_plugin_id == pfring_plugin_id)
          rule->rule.extended_fields.filter_plugin_id = NO_PLUGIN_ID;
      }
    }
...
register_pfring_plugin get_register_pfring_plugin(void)
{
  return register_plugin;
}
EXPORT_SYMBOL(get_register_pfring_plugin);

unregister_pfring_plugin get_unregister_pfring_plugin(void)
{
...
{
  /* Kept for compatibility with old plugin APIs. */
  if(the_handler && the_handler != register_plugin)
    printk("[PF_RING] custom register_pfring_plugin handler ignored\n");
}
                           ...
                           int optlen)
{
  ...
  case SO_SET_STACK_INJECTION_MODE:
    if(optlen > sizeof(pfr->kernel_consumer_plugin_id)) {
      u_int diff = optlen - sizeof(pfr->kernel_consumer_plugin_id);
      u_char *consumer_config = kmalloc(optlen, GFP_KERNEL);

      if(consumer_config == NULL)
        return(-EFAULT);

      if(copy_from_sockptr(consumer_config, optval, optlen)) {
        kfree(consumer_config);
        return(-EFAULT);
      }
      ...
      memcpy(&pfr->kernel_consumer_plugin_id, consumer_config,
      ...
      if(diff > 0) {
        ...
        memcpy(pfr->kernel_consumer_options,
               consumer_config + sizeof(pfr->kernel_consumer_plugin_id),
               diff);
      }
      ...
      if(pfr->kernel_consumer_plugin_id
         && plugin_registration[pfr->kernel_consumer_plugin_id] != NULL
         && plugin_registration[pfr->kernel_consumer_plugin_id]->pfring_packet_start != NULL
         && (!pfr->ring_active)) {
          ...
          while(bucket != NULL) {
            if(hash_bucket_match_rule(bucket, &rule)) {
              ...
              if(bucket->rule.plugin_action.plugin_id > 0
                 && bucket->rule.plugin_action.plugin_id < MAX_PLUGIN_ID
                 && plugin_registration[bucket->rule.plugin_action.plugin_id] != NULL
                 && plugin_registration[bucket->rule.plugin_action.plugin_id]->pfring_plugin_get_stats != NULL) {
                char *buffer = kmalloc(len, GFP_ATOMIC);

                if(buffer == NULL) {
                  rc = -EFAULT;
                } else {
                  rc = plugin_registration[bucket->rule.plugin_action.plugin_id]->pfring_plugin_get_stats(pfr, NULL, bucket, buffer, len);
                  if(rc > 0 && copy_to_user(optval, buffer, rc))
                    rc = -EFAULT;
                  kfree(buffer);
                }
                break;
        ...
        if(rule->rule.rule_id == rule_id) {
          ...
          if(rule->rule.plugin_action.plugin_id > 0
            ...
            if(buffer == NULL) {
              ...
              rc = plugin_registration[rule->rule.plugin_action.plugin_id]->pfring_plugin_get_stats(pfr, rule, NULL, buffer, len);
              if(rc > 0 && copy_to_user(optval, buffer, rc))
                rc = -EFAULT;
              kfree(buffer);
            }
...
static int __init ring_init(void)
{
  ...
  if(quick_mode != 0)
    printk("[PF_RING] Warning: quick_mode is deprecated!\n");
