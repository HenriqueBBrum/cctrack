...
static const struct proc_ops proc_cctrack_stats_operations = {
		.proc_open    = cctrack_open,
		.proc_read    = seq_read,
...
static void cctrack_seq_stop(struct seq_file *s, void *v)
{
	...
	if(v != NULL){
		...
		//cctrack_assertert((s64)cctrack_stats_generic->pos >= (s64)spos);
...
static int cctrack_seq_show(struct seq_file *s, void *v)
{
	...
	//cctrack_assertert(s->private == NULL);
	//cctrack_assertert(v != NULL);

	//printk("cctrack_seq_show %lld \n", pos);

	//cctrack_assertert(pos < CCTRACK_STATS_GENERIC_MAX);

	//cctrack_assertert(pos < cctrack_stats_generic->pos);

	i = pos;
	//cctrack_assertert(i == pos);
	...
	//cctrack_assertert(num_pkts_sampled >= 0);
	//cctrack_assertert(num_pkts >= 0);
	//cctrack_assertert(num_pkts_below_limit >= 0);
	...
	//cctrack_assertert(num_pkts + 32 >= num_pkts_sampled);
	//cctrack_assertert(num_pkts + 32 >= num_pkts_below_limit);
	...
	// cctrack_assertert((cctrack_stats_generic->stats[i].flags == 1) ?
...
int init_cctrack_stats_generic(void){
	...
	//cctrack_assertert(cctrack_stats_generic->pos == 0);
...
void cctrack_stats_generic_add_every_pkt(struct timeval *ts)
{
	...
	//cctrack_assertert(pos >= 0 && pos <= CCTRACK_STATS_GENERIC_MAX);
		...
		uint32_t sampling_limit)
{
	...
	//cctrack_assertert(sl_pos >= 0 && sl_pos < __CCTRACK_STATS_GENERIC_SL_FIELD);
